...
- Regenerate with `pnpm record:generate`. The Pass state and Judge experiments intentionally exit `1`; only the checked helper may accept that exact status and its named output evidence. The standalone Score skip succeeds with exit `0` and must still expose its named output evidence.
